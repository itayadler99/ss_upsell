...
const ONE_PAIR_PRICE = 299;
const TWO_PAIR_PRICE = 499;
...
// Per-render event log.
//
// `seen` only answers "did any screen render in the last six hours", which is a
// measure of our own output, not of the customer: one render and forty renders look
// identical. With zero sales across 367 orders the question that actually matters is
// how many buyers were shown a pair at all, and that number did not exist anywhere.
// Every call to /api/offer and /api/sign appends one row here, capped so the metafield
// stays small. referenceId is kept so a row can be joined back to the order - and
// therefore to its payment gateway - offline, without slowing the buyer down.
...
async function logEvent(evt) {
  ...
  list.push({ t: new Date().toISOString(), ...evt });
  const next = list.slice(-EVENTS_CAP);
  await gql(
    `mutation($m:[MetafieldsSetInput!]!){ metafieldsSet(metafields:$m){ userErrors{ message } } }`,
    {
      m: [
...
async function readEvents() {
  ...
  try {
    const v = JSON.parse((d.shop.metafield && d.shop.metafield.value) || '[]');
...
// What the buyer is told the size is. The site keeps Adidas in thirds ("37 1/3",
// "44 2/3"), which nobody shops by and which makes the offer read like a mistake.
// Itay's call: show the round size, so a third drops to the whole number below it
// (37 1/3 -> 37, 44 2/3 -> 44) while real half sizes stay untouched. Display only -
// the variant added to the cart is the site's, unchanged.
function formatSize(n) {
  const num = typeof n === 'number' ? n : parseSize(n);
  if (num === null) return String(n || '');
  const twice = num * 2;
  if (Number.isInteger(twice)) return Number.isInteger(num) ? String(num) : num.toFixed(1);
