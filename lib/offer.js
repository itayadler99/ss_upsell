...
const APP_URL = process.env.APP_URL || 'https://ss-upsell.vercel.app';
...
async function homeStockOffer(shopDomain, purchase, size) {
  ...
  // The image is routed through /api/px so that a paint by the customer's browser is
  // observable. See api/px.js - it is the only way to tell "screen never shown" apart
  // from "screen shown and ignored" without shipping a new extension bundle.
  const beacon = (src) => {
    const real = img(src);
    if (!real) return null;
    const u = Buffer.from(real, 'utf8').toString('base64url');
    const ref = encodeURIComponent(String(purchase.referenceId || ''));
    return `${APP_URL}/api/px?u=${u}&ref=${ref}&shop=${encodeURIComponent(shopDomain)}`;
  };
  const items = cands.slice(0, 4).map((c) => {
    ...
    return {
      ...
      title: c.title,
      handle: c.handle,
      image: beacon(sh.image),
      imageDirect: img(sh.image),
